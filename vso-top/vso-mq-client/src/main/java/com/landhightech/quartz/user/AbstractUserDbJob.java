 ...
 *
 */
public abstract class AbstractUserDbJob {
	private final Logger logger = LoggerFactory.getLogger(AbstractUserDbJob.class);
	protected boolean doAddUser(String message,boolean isTimer){
		User user = null;
		Long id = null;
		...
		if(id!=null){
			logger.info("user register sucess:{}",id);
			isSuccess = true;
		}
		if(id == null){
			if(!isTimer)
				logger.info("user register fail:{}",message);
